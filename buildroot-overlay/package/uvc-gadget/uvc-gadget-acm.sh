...
create_frame() {
    ...
    case "$FORMAT" in
        ...
        mjpeg)
            ...
            case "$WIDTH" in
                1920)
                    echo "300000"  >  $wdir/dwMaxVideoFrameBufferSize
                    echo "80000000" > $wdir/dwMaxBitRate
                    echo "5000000"  > $wdir/dwMinBitRate
                ;;
                1280)
                    echo "150000" >  $wdir/dwMaxVideoFrameBufferSize
                    echo "80000000" > $wdir/dwMaxBitRate
                    echo "3000000"  > $wdir/dwMinBitRate
                ;;
                640)
                    echo "50000" >  $wdir/dwMaxVideoFrameBufferSize
                    echo "80000000" > $wdir/dwMaxBitRate
...
start_uvc_gadget() {
    ...
    echo "UVC+ACM Gadget" > strings/0x409/product
    ...
    # 6. 创建 ACM 功能实例（使用相对路径）
    mkdir -p functions/acm.usb0

    # 7. 创建配置项（在所有 function 创建完之后）
    mkdir -p configs/c.1/strings/0x409
    echo "Config 1 (UVC+ACM)" > configs/c.1/strings/0x409/configuration
    echo 500 > configs/c.1/MaxPower

    # 8. 绑定所有 function 到配置
    ln -s ${FUNCTION} configs/c.1/uvc.usb0      # UVC function
    ln -s functions/acm.usb0 configs/c.1/acm.usb0       # ACM function

    # 9. 启动！
    echo "91500000.usb" > UDC

    echo "UVC+ACM gadget started"
    echo "UVC: /dev/video* | ACM: /dev/ttyGS0"
}
...
stop_uvc_gadget() {
    ...
    # 3. 删除 function 链接
    rm -f "configs/c.1/uvc.usb0" 2>/dev/null
    rm -f "configs/c.1/acm.usb0" 2>/dev/null
    ...
    rmmod usb_f_acm 2>/dev/null || true
