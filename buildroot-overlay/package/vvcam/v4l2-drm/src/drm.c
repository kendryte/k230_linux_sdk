...
static int drm_dmabuf_set_plane(struct drm_buffer *buf)
{
	...
	drm_add_plane_property("SRC_W", drm_dev.buffer_width << 16);
	drm_add_plane_property("SRC_H", drm_dev.buffer_height << 16);
	drm_add_plane_property("CRTC_X", 0);
	drm_add_plane_property("CRTC_Y", 0);
	drm_add_plane_property("CRTC_W", drm_dev.buffer_width);
	drm_add_plane_property("CRTC_H", drm_dev.buffer_height);
...
static int drm_setup(unsigned int fourcc)
{
	...
	info("drm: %ux%u (%u mm X %u mm) pixel format %c%c%c%c",
...
static int drm_allocate_dumb(struct drm_buffer *buf)
{
	...
	creq.width = drm_dev.buffer_width;
	creq.height = drm_dev.buffer_height;
    switch (drm_dev.fourcc) {
        ...
        case DRM_FORMAT_BGRA8888:
            ...
            break;
        default:
            print("Unknown plane format fourcc %08x", drm_dev.fourcc);
            return -1;
    }
	...
	dbg("pitch %u", buf->pitch);
	buf->size = creq.size;
	dbg("size %lu", buf->size);
    ...
    buf->fd = prime_handle.fd;

	/* create framebuffer object for the dumb-buffer */
	handles[0] = creq.handle;
	pitches[0] = creq.pitch;
	offsets[0] = 0;
    if ((drm_dev.fourcc == DRM_FORMAT_NV12) || (drm_dev.fourcc == DRM_FORMAT_NV21)) {
        handles[1] = creq.handle;
        pitches[1] = pitches[0];
        offsets[1] = pitches[0] * drm_dev.buffer_height;
    }
	ret = drmModeAddFB2(drm_dev.fd, drm_dev.buffer_width, drm_dev.buffer_height, drm_dev.fourcc,
...
static int drm_setup_buffers(void)
{
    ...
    for (unsigned i = 0; i < drm_dev.buffer_num; i++) {
...
int drm_get_dmabuf_fd(unsigned index) {
	return drm_dev.drm_bufs[index].fd;
}
...
int drm_init(unsigned int fourcc, unsigned int buffer_num, uint32_t width, uint32_t height)
{
    ...
    drm_dev.buffer_num = buffer_num;
	if (width == 0) {
		drm_dev.buffer_width = drm_dev.width;
	} else {
		drm_dev.buffer_width = width;
	}
	if (height == 0) {
		drm_dev.buffer_height = drm_dev.height;
	} else {
    	drm_dev.buffer_height = height;
	}
    drm_dev.drm_bufs = malloc(sizeof(struct drm_buffer) * buffer_num);
