...
void sctp_reset(Sctp* sctp) {
  if (!sctp) return;

  /* Save callbacks — they are set once by peer_connection_ondatachannel
     and must survive across reconnections */
  void (*onmessage)(char*, size_t, void*, uint16_t) = sctp->onmessage;
  void (*onopen)(void*) = sctp->onopen;
  void (*onclose)(void*) = sctp->onclose;
  void* userdata = sctp->userdata;
