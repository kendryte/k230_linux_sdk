...
public class CachedNearlinkDeviceManager {
    ...
    public static boolean onDeviceDisappeared(CachedNearlinkDevice cachedDevice) {
        ...
        return cachedDevice.getBondState() == NearlinkConstant.SLE_PAIR_NONE;
    }
    ...
    public synchronized void clearNonBondedDevices() {
        mCachedDevicesMapForHearingAids.entrySet().removeIf(entries
                -> entries.getValue().getBondState() == NearlinkConstant.SLE_PAIR_NONE);
        mCachedDevices.removeIf(cachedDevice
                -> cachedDevice.getBondState() == NearlinkConstant.SLE_PAIR_NONE);
    }
    ...
    public synchronized void onNearlinkStateChanged(int nearlinkState) {
        ...
        if (nearlinkState == NearlinkAdapter.STATE_TURNING_OFF) {
            for (int i = mCachedDevices.size() - 1; i >= 0; i--) {
                CachedNearlinkDevice cachedDevice = mCachedDevices.get(i);
                if (cachedDevice.getBondState() != NearlinkConstant.SLE_PAIR_PAIRED) {
                    cachedDevice.setJustDiscovered(false);
