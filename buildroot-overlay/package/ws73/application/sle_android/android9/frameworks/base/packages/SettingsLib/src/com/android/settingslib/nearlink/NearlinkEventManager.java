...
public class NearlinkEventManager {
    ...
    private class DeviceFoundHandler implements Handler {
        public void onReceive(Context context, Intent intent, NearlinkDevice device) {
            ...
            String aliName = intent.getStringExtra(NearlinkDevice.EXTRA_ALIAS);
            ...
            if (!TextUtils.isEmpty(aliName)) {
                cachedDevice.setNewName(aliName);
            }
    ...
    boolean readPairedDevices() {
        ...
        for (NearlinkDevice device : bondedDevices) {
            ...
            if (cachedDevice == null) {
                cachedDevice = mDeviceManager.addDevice(mLocalAdapter, mProfileManager, device);
                cachedDevice.setNewName(device.getAliasName());
