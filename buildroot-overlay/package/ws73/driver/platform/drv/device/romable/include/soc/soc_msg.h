...
typedef struct {
    td_u32 msg_id; /* < Message ID.CNcomment:消息ID CNend */
...
* @param  queue_len    [IN]  type  #td_u16，Message queue length, that is, the number of messages that can be stored
*                      in the message queue. The value range is [1,0xffff].
CNcomment:消息队列长度，即该消息队列支持存储多少条消息。取值范围为[1,0xffff]。CNend
* @param  msg_size     [IN]  type  #td_u32，Size of each message in the message queue (unit: byte).
                       The value range is [1,0xffff-4].
CNcomment:消息队列中一条消息大小（单位：byte）。取值范围为[1,0xffff-4]。CNend
*
* @retval #EXT_ERR_SUCCESS           Success
* @retval #EXT_ERR_MSG_INVALID_PARAM An input argument is incorrect, the handle pointer is null, the name address is
*         null, or the message queue length is 0.CNcomment:入参错误、句柄指针为空、名字地址为空、消息队列长度为0。CNend
* @retval #EXT_ERR_MSG_CREATE_Q_FAIL An error occurred when creating the message queue, for example, insufficient
*         memory or insufficient message queue resources.
CNcomment:创建消息队列错误，比如：内存不足、消息队列资源不足。CNend
* @par 依赖:
*            @li soc_msg.h：Describes message queue APIs.CNcomment:文件用于描述消息队列相关接口。CNend
*            @li soc_errno.h：Describes file configuration error codes.CNcomment:文件配置错误码。CNend
* @see uapi_msg_queue_delete。
*/
ext_errno uapi_msg_queue_create(TD_OUT td_u32 *id, td_u16 queue_len, td_u32 msg_size);
...
*
* @attention None
* @param  id           [IN] type  #td_u32，Handle of the created message queue.CNcomment:消息队列句柄。CNend
*
* @retval #EXT_ERR_SUCCESS           Success
* @retval #EXT_ERR_MSG_Q_DELETE_FAIL An error occurred with the message queue, for example, the ID is out of
...
*/
ext_errno uapi_msg_queue_delete(td_u32 id);
...
*
* @attention None
* @param  id           [IN] type #td_u32，Message queue handle.CNcomment:消息队列句柄。CNend
* @param  msg          [IN] type #td_pvoid，Message content pointer.CNcomment:消息内容指针。CNend
* @param  timeout_ms   [IN] type #td_u32，Timeout period for sending a message. The value 0 indicates
...
*
* @retval #EXT_ERR_SUCCESS           Success.
* @retval #EXT_ERR_MSG_SEND_FAIL     An error occurs with the message queue, for example, an input argument is
...
中断中使用但超时时间不为0。CNend
*
* @retval #EXT_ERR_MSG_INVALID_PARAM An input argument is incorrect or the message queue pointer is null.
...
*/
ext_errno uapi_msg_queue_send(td_u32 id, td_pvoid msg, td_u32 timeout_ms, td_u32 msg_size);
...
* @attention None
* @param  id          [IN]   type #td_u32，Message queue handle.CNcomment:消息队列句柄。CNend
* @param  msg         [OUT]  type #td_pvoid，Message content pointer.CNcomment:消息内容指针。CNend
* @param  timeout_ms  [IN]   type #td_u32，Timeout period for receiving a message. The value #EXT_SYS_WAIT_FOREVER
*                     indicates permanent wait.CNcomment:消息接收超时时间(单位：ms)，
如果表示永久等待使用#EXT_SYS_WAIT_FOREVER。CNend
* @param  msg_size    [IN]   type #td_u32*，Expected message length (unit: byte),if wait msg success, this val will fill
with actually size of msg.
CNcomment:输入期望的消息长度（单位：byte），如果等待消息成功，该值将被赋值为实际接收到的消息长度。CNend
*
* @retval #EXT_ERR_SUCCESS           Success.CNcomment:消息接收成功。CNend
* @retval #EXT_ERR_MSG_WAIT_FAIL     An error occurs with the message queue, for example,
                                     an input argument is incorrect,
...
* @retval #EXT_ERR_MSG_INVALID_PARAM An input argument is incorrect or the message queue pointer is null.
CNcomment:入参错误、消息队列指针为空。CNend
* @retval #EXT_ERR_MSG_WAIT_TIME_OUT No message is received when the waiting times out.
...
*/
ext_errno uapi_msg_queue_wait(td_u32 id, TD_OUT td_pvoid msg, td_u32 timeout_ms, td_u32* msg_size);
...
* @param  id        [IN] type #td_u32，Message queue handle.CNcomment:消息队列句柄。CNend
*
* @retval #EXT_TRUE  The message queue is full or the message queue information fails to be obtained.
CNcomment:消息队列已满或获取消息队列信息失败。CNend
* @retval #EXT_FALSE The message queue is not full.CNcomment:消息队列未满。CNend
* @par 依赖:
*            @li soc_msg.h：Describes message queue APIs.CNcomment:文件用于描述消息队列相关接口。CNend
* @see None
*/
td_bool uapi_msg_queue_is_full(td_u32 id);
...
*
* @attention None
* @param  id       [IN] #td_u32，Handle of the created message queue.CNcomment:消息队列句柄。CNend
*
* @retval #EXT_ERR_MSG_INVALID_MSG_NUM  Failed to read the message queue.CNcomment:读取消息队列出错。CNend
* @retval value                        Number of used message queues.CNcomment:消息队列使用个数。CNend
* @par 依赖:
*            @li soc_msg.h：Describes message queue APIs.CNcomment:文件用于描述消息队列相关接口。CNend
* @see None
*/
td_u32 uapi_msg_queue_get_msg_num(td_u32 id);
...
* @param  id       [IN] #td_u32，Handle of the created message queue.CNcomment:消息队列句柄。CNend
*
* @retval #EXT_ERR_MSG_INVALID_MSG_NUM  An error occurs with the message queue. For example: An input argument is
...
* @see None
*/
td_u32 uapi_msg_queue_get_msg_total(td_u32 id);
...
* @see uapi_msg_queue_wait。
*/
ext_errno uapi_msg_queue_send_msg_to_front(td_u32 id,  td_pvoid msg, td_u32 msg_size, td_u32 timeout_ms);
