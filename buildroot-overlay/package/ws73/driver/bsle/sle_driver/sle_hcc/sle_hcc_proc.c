...
static sle_hcc_rx g_sle_hcc_rx = NULL;
...
#endif
    if (*buf == TD_NULL) {
        printk(KERN_ERR"sle_hcc_adapt_alloc failed\r\n");
...
static td_u32 sle_hcc_adapt_rx(hcc_queue_type queue_id, td_u8 sub_type, td_u8 *buf, td_u32 len, td_u8 *user_param)
{
    ...
    if (get_sle_state() == SLE_OFF) {
        osal_kfree(user_param);
        printk(KERN_ERR"sle hcc adapt rx sle closed \n");
        return EXT_ERR_FAILURE;
    }

    if (sle_hci_send_to_chba(buf, len) == 0) {
        if (g_sle_hcc_rx != NULL) {
            g_sle_hcc_rx(buf, len);
        }
    }
...
int sle_hci_send_frame(uint8_t *data_buf, uint16_t len)
{
    ...
    if (get_sle_state() == SLE_OFF) {
        printk(KERN_ERR"sle hci send frame sle state closed\n");
        return EXT_ERR_FAILURE;
    }

    if (len > SLE_TX_MAX_FRAME) {
        printk(KERN_ERR"sle send bug data len is too large, len=%u\n", len);
        return EXT_ERR_FAILURE;
    }
    ...
    if (ret != EXT_ERR_SUCCESS) {
        printk(KERN_ERR"send tx data failed ret: 0x%x\n", ret);
    }
    return ret;
}
