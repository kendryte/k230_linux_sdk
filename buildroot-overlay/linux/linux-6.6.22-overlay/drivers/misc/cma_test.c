...
cma_test_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < CMA_NUM; i++) {
		if (dma_virt[i]) {
			dma_free_coherent(cma_dev, CMA_BUFFER_SIZE, dma_virt[i], dma_phys[i]);
			_dev_info(cma_dev, "free virt: %p phys: %llX\n", dma_virt[i], dma_phys[i]);
			dma_virt[i] = NULL;
			// break;
		}
	}
...
cma_test_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < CMA_NUM; i++) {
		if (!dma_virt[i]) {
			dma_virt[i] = dma_alloc_coherent(cma_dev, CMA_BUFFER_SIZE, &dma_phys[i], GFP_KERNEL);
 
			if (dma_virt[i]) {
				void *p;
				/* touch every page in the allocated memory */
				for (p = dma_virt[i]; p < dma_virt[i] + CMA_BUFFER_SIZE; p += 4)
					*(u32 *)p = i+1;
 
				_dev_info(cma_dev, "alloc virt: %p phys: 0x%llX\n", dma_virt[i], dma_phys[i]);
			} else {
