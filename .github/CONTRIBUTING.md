...
1. 打开issue界面 https://github.com/kendryte/k230_linux_sdk/issues
...
1. Fork本仓库到您的github
2. Clone仓库到您本地
3. 您可以基于`dev`分支创建新分支并添加更改`git checkout -b newbranch -l dev`
4. 提交您的更改到您的github
5. 进入您的github仓库点击`Pull request`按钮后点击`New pull request`按钮
6. 选择`base repository`为`kendryte/k230_linux_sdk`，选择`base`为`dev`，选择`head repository`为`yourname/k230_linux_sdk`，选择`compare`为`newbranch`
